...
const contractGen = require('truffle-contract')
const MetaCoinArtifact = require('../build/contracts/Metacoin')
const MetCoinContract = contractGen(MetaCoinArtifact)
...
const allFns = MetaCoinArtifact.abi
                .filter(item => item.type === 'function')
                .map(item => { return { contract: MetCoinContract, fnName: item.name }})

const allResolvers = allFns.reduce((output, input) => {
