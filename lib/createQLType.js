...
const convertTypeToGraphQLName = (type, index) => {
  let name = type
  if (type.includes('[]')) {
    name = `${type.substring(0, type.length - 2)}Arr_${index}`
  } else {
    name = `${type}_${index}`
  }
  return name
...
const abiOutputToGraphQLType = (input, index) => {
  ...
  const name = convertTypeToGraphQLName(input.type, index)
...
const genOutputFuncitonMapper = (input) => {
  return (data) => {
    ...
    const tmp = input.reduce((out, current, index) => {
      // console.log(current)
      // console.log(index)

      if (!Array.isArray(data)) {
        // console.log(`what type is the single result? ${typeof data}`)
